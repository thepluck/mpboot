...
void addPerSiteSubtreeScores(partitionList *pr, int pNumber, int qNumber,
  ...
  //PLL_PCF; 		pBuf = &(pr->partitionData[i]->perSitePartialPars[partialParsLength
  //* pNumber]); 		qBuf =
  //&(pr->partitionData[i]->perSitePartialPars[partialParsLength * qNumber]);
  //		rBuf = &(pr->partitionData[i]->perSitePartialPars[partialParsLength
  //* rNumber]); 		for(int k = 0; k < partialParsLength; k++) 			pBuf[k] += qBuf[k] +
  //rBuf[k];
  //	}
...
//        return;
    ...
    if (globalParam->sankoff_short_int) {
      ...
      case 20:
        ...
        break;
      case 2:
        newviewSankoffParsimonyIterativeFastSIMD<Vec8ui, parsimonyNumber, 2>(
            tr, pr);
        break;
      case 32:
        newviewSankoffParsimonyIterativeFastSIMD<Vec8ui, parsimonyNumber, 32>(
            tr, pr);
        break;
      default:
        cerr << "Unsupported" << endl;
    ...
    if (globalParam->sankoff_short_int) {
      ...
      case 20:
        ...
        break;
      case 2:
        newviewSankoffParsimonyIterativeFastSIMD<Vec4ui, parsimonyNumber, 2>(
            tr, pr);
        break;
      case 32:
        newviewSankoffParsimonyIterativeFastSIMD<Vec4ui, parsimonyNumber, 32>(
            tr, pr);
        break;
      default:
        cerr << "Unsupported" << endl;
...
//        return evaluateSankoffParsimonyIterativeFast(tr, pr, perSiteScores);
    ...
    if (globalParam->sankoff_short_int) {
      ...
      case 2:
        return evaluateSankoffParsimonyIterativeFastSIMD<
            Vec16us, parsimonyNumberShort, 2, true>(tr, pr, perSiteScores);
      case 32:
        return evaluateSankoffParsimonyIterativeFastSIMD<
            Vec16us, parsimonyNumberShort, 32, true>(tr, pr, perSiteScores);
      default:
        cerr << "Unsupported" << endl;
        exit(EXIT_FAILURE);
      }
    ...
    if (globalParam->sankoff_short_int) {
      ...
      case 2:
        return evaluateSankoffParsimonyIterativeFastSIMD<
            Vec8us, parsimonyNumberShort, 2, true>(tr, pr, perSiteScores);
      case 32:
        return evaluateSankoffParsimonyIterativeFastSIMD<
            Vec8us, parsimonyNumberShort, 32, true>(tr, pr, perSiteScores);
      default:
        cerr << "Unsupported" << endl;
        exit(EXIT_FAILURE);
      }
...
static void reorderNodes(pllInstance *tr, nodeptr *np, nodeptr p, int *count) {
  ...
  if ((p->number <= tr->mxtips))
    ...
    for (i = tr->mxtips + 1;
         (i <= (tr->mxtips + tr->mxtips - 1)) && (found == 0); i++) {
...
static void testInsertParsimony(pllInstance *tr, partitionList *pr, nodeptr p,
  ...
  if (doIt) {
    double *z = (double *)rax_malloc(
        numBranches *
        sizeof(
            double)); // Diep: copy from the latest pllrepo (fastDNAparsimony.c)
...
static int rearrangeParsimony(pllInstance *tr, partitionList *pr, nodeptr p,
  ...
  if ((p->number > tr->mxtips) && doP) {
    ...
    if ((p1->number > tr->mxtips) || (p2->number > tr->mxtips)) {
      ...
      if ((p1->number > tr->mxtips)) {
        addTraverseParsimony(tr, pr, p, p1->next->back, mintrav, maxtrav, doAll,
                             PLL_FALSE, perSiteScores);
        addTraverseParsimony(tr, pr, p, p1->next->next->back, mintrav, maxtrav,
                             doAll, PLL_FALSE, perSiteScores);
      }

      if ((p2->number > tr->mxtips)) {
        addTraverseParsimony(tr, pr, p, p2->next->back, mintrav, maxtrav, doAll,
                             PLL_FALSE, perSiteScores);
        addTraverseParsimony(tr, pr, p, p2->next->next->back, mintrav, maxtrav,
                             doAll, PLL_FALSE, perSiteScores);
      }
  ...
  if ((q->number > tr->mxtips) && (maxtrav > 0) && doQ) {
    ...
    if (((q1->number > tr->mxtips) &&
      ...
      if ((q1->number > tr->mxtips)) {
        addTraverseParsimony(tr, pr, q, q1->next->back, mintrav2, maxtrav,
                             doAll, PLL_FALSE, perSiteScores);
        addTraverseParsimony(tr, pr, q, q1->next->next->back, mintrav2, maxtrav,
                             doAll, PLL_FALSE, perSiteScores);
      }

      if ((q2->number > tr->mxtips)) {
        addTraverseParsimony(tr, pr, q, q2->next->back, mintrav2, maxtrav,
                             doAll, PLL_FALSE, perSiteScores);
        addTraverseParsimony(tr, pr, q, q2->next->next->back, mintrav2, maxtrav,
                             doAll, PLL_FALSE, perSiteScores);
      }
        ...
        if (informative[index]) {
          ...
          */
          ...
          for (k = 0; k < states; k++) {
            ...
            //compressedValues[k]; // Diep 					  cout << "compressedValues[k]: " <<
...
static void _pllMakeParsimonyTreeFast(pllInstance *tr, partitionList *pr,
  ...
  do {
    ...
    //		for(j = 1; j <= tr->mxtips + tr->mxtips - 2; j++)
    {
      ...
      // cout << setprecision(10) << fixed << getCPUTime() - startTime << '\n';
...
void _pllComputeRandomizedStepwiseAdditionParsimonyTree(
  ...
  // cout << setprecision(10) << getCPUTime() - startTime << '\n';
  //	cout << "DONE make...." << endl;
...
int pllOptimizeSprParsimony(pllInstance *tr, partitionList *pr, int mintrav,
  ...
  //&& (!globalParam->hclimb1_nni)) || (!iqtree)){ 		iqtree = _iqtree;
  //		// consider updating tr->yVector, then tr->aliaswgt (similar as in
  //pllLoadAlignment) 		if((globalParam->ratchet_iter >= 0  ||
  //globalParam->optimize_boot_trees) && (!globalParam->hclimb1_nni))
  //			_updateInternalPllOnRatchet(tr, pr);
  //		_allocateParsimonyDataStructures(tr, pr, perSiteScores); // called
  //once if not running ratchet
  //	}
...
void pllComputePatternParsimony(pllInstance *tr, partitionList *pr,
  ...
  for (int i = 0; i < pr->numberOfPartitions; i++) {
    ...
    for (ptn = pr->partitionData[i]->lower; ptn < pr->partitionData[i]->upper;
         ptn++) {
...
void testSiteParsimony(Params &params) {
  ...
  //params.intype); 	iqtree->setAlignment(&alignment); 	iqtree->setParams(params);
  //
  ...
  //params.model_name.find_first_of("+{")); 		else 			model = "WAG"; 	} else {
  ...
  //iqtree->pllPartitions)) { 		printf("Incompatible tree/alignment
  //combination\n"); 		exit(1);
  //	}
  //
  ...
  //PLL_TRUE);   // the last parameter is that you want to allocate buffers for
  ...
  //iqtree->pllInst->start->back, PLL_TRUE, PLL_TRUE)); 	int sum_test = 0; 	int
  //zero_count = 0; 	for (i = 0; i < iqtree->pllPartitions->numberOfPartitions;
  //++i)
  //	{
  //		cout << "parsimonyLength of partition " << i << " is " <<
  //iqtree->pllPartitions->partitionData[i]->parsimonyLength << endl; 		for (j =
  //0; j < iqtree->pllPartitions->partitionData[i]->parsimonyLength * PLL_PCF;
  //++j){ 			cout << iqtree->pllPartitions->partitionData[i]->perSiteParsScores[j]
  //<< " "; 			sum_test +=
  ...
  for (p = 0; p < pl->numberOfPartitions; ++p) {
    ...
    for (i = 0; i < pl->partitionData[p]->width; ++i) {
      for (j = 0; j < alignmentData->sequenceCount; ++j) {
        sites[p][i][j] =
            alignmentData->sequenceData[j + 1][pl->partitionData[p]->lower + i];
      }
      sites[p][i][j] = 0;
    }
